...
memoma3は、キャンバス上にメモ（オブジェクト）を自由に配置・移動でき、接続線やグループで整理しながらアイデアをとりまとめていくことを目的としたアプリです。

- **開発言語**: Flutter (Dart)
- **状態管理**: Riverpod (3.x)
- **対応プラットフォーム**: Windows / Web / Android / Linux
- **ファイル入出力**: `file_picker`（クロスプラットフォーム対応）
- **オブジェクト編集**: 14 形状・強調レベル・ラベル・本文・説明・本体色・ラベル色・説明色・サイズ（0.5〜4倍）・ラベル/説明の文字サイズ（1〜5段階）の編集に対応
- **オブジェクト複製**: 選択分を複製（Ctrl+D / アクションバー / 一覧の「複製」ボタン）
- **カラーピッカー**: `flex_color_picker` をラップした `MyCustomColorPicker`（カラーホイール・シェード選択・カラーコード・コピー＆ペースト・OK/キャンセル）
- **接続線**: 2 つのオブジェクトを線でつなぐ（線種 4 種・形状 6 種・色設定・接続解除）
- **グループ化**: 複数のオブジェクトを囲むグループ枠（名称・説明・枠線の色・枠ごと移動）
- **整列**: 1 個選択で X/Y を 10 の倍数に、複数選択で左/右/上/下/等間隔（横幅・縦幅）（サイズ倍率を考慮）
- **選択モード**: キーボードのない環境（Android 等）向けの複数選択（タップで選択トグル・全選択）
- **Undo / Redo**: 最大 30 件まで戻す・やり直す
- **画像 / PDF エクスポート**: キャンバスを PNG / PDF に書き出し
- **オブジェクト一覧**: 全オブジェクトの属性を表示・フィルタ・ソートし、CSV エクスポート（サイズ列を含む）
...
`InteractiveViewer`（`TransformationController` 付き）により、キャンバス全体のパン（ドラッグ）とズーム（ピンチ）が可能です。
...
- **ドラッグ**: オブジェクトを移動します。ドラッグ中はローカル表示座標が即座に更新され、移動量は現在の拡大率で割られるため、ズーム状態に関係なく自然に追従します。
- **最前面移動**: オブジェクトをタップ／ドラッグ開始した際、そのオブジェクトが Z-Index のトップ（リスト末尾）へ移動します。
- **編集**: 上部アクションバーの「編集」ボタン、またはオブジェクトのダブルタップ／長押しで、選択中のオブジェクトの形状・ラベル・詳細・説明・色・ラベル色・説明色・サイズ（0.5〜4倍）・ラベル/説明の文字サイズ（1〜5段階）を変更できます。
- **複製**: 上部アクションバーの「複製」ボタン、またはキーボードショートカット `Ctrl + D` で、選択中のオブジェクトを複製します（右下 20px ずらし、接続線・グループは引き継がない）。
...
キーボードが接続されていない環境（Android 等）では Ctrl+タップが使えない状況を想定し、上部アクションバーの「選択モード」ボタンで選択モードを有効にすると、オブジェクトが複数選択できます。
...
| 複製（ツールバー） | 「複製」ボタン / `Ctrl + D` | 選択中のオブジェクトを複製（右下 20px ずらし） |
...
- **保存**: 全オブジェクトを JSON 文字列として保存します。
  - Windows / Linux / Android: `file_picker` の `saveFile` で保存先を選択。
  - Web: 保存ダイアログのフィールドに JSON 文字列を表示し、コピー／保存します。
- **読み込み**: JSON を読み込んでキャンバス状態を復元します。
  - Windows / Linux / Android: `file_picker` の `pickFiles` でファイルを選択。
...
- **表示列**: No. / 名称 / 説明 / 詳細 / グループ / 形状 / 強調 / 色 / 接続(from/to) / X / Y / サイズ / 中心 / 複製 / 削除
- **フィルタ**: 名称（テキスト）・形状（複数選択）・強調（複数選択）
- **ソート**: 列ヘッダクリックで昇降順切替（サイズ列もソート可能）
- **中心移動**: 各行のボタンで、そのオブジェクトの中心座標をキャンバス中心に設定
- **複製**: 各行のボタンで、そのオブジェクトを複製（右下 20px ずらし）
- **CSV エクスポート**: 全オブジェクトの属性を CSV（UTF-8 BOM 付き、サイズ列を含む）で出力。仕様は [`docs/CSV.md`](docs/CSV.md) を参照
...
| ファイル | クラス | 役割 |
| --- | --- | --- |
| `models/note_object.dart` | `NoteObject` | オブジェクトのデータモデル（不変）。`toJson()`/`fromJson()`/`copyWith()`/`rectInCanvas()` を実装。14 形状・強調レベル・ラベル色・説明色・サイズ倍率（0.5〜4倍）・ラベル/説明の文字サイズレベル（1〜5）に対応。 |
...
> **注**: `flutter` が PATH に登録されていない場合は、`flutter.bat` をフルパスで実行してください。（詳細は [`docs/DEVELOPMENT.md`](docs/DEVELOPMENT.md) を参照してください。）
...
Copyright 2026- GOKIGEN Project  Licensed under the Apache License, Version 2.0
